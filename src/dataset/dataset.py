...
desc_str = r"""Create train and ground-truth images suitable
...
two subdirectories: "in" with train images and "gt" with ground-truth images.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=r'directory with input and gt images'
    )
    parser.add_argument(
        "-o",
        "--output",
        ...
        help=r'directory for output images suitable for U-net',
    )
    parser.add_argument(
        "-s",
        "--shuffle",
        action="store_true",
        help=r"shuffle images if You are using different datasets as one)",
    )
    parser.add_argument(
        "--size_x",
        type=int,
        default=128,
        help=r"x size of image part",
    )
    parser.add_argument(
        "--size_y",
        type=int,
        default=128,
        help=r"y size of image part",
    )
    parser.add_argument(
        "--step_x", type=int, default=128, help=r"x step"
    )
    parser.add_argument(
        "--step_y",
        type=int,
...
def main():
    ...
    for i, fname in enumerate(
        ...
        copy2(
            os.path.join(fname),
            os.path.join(args.output, "in", str(i) + "_in.png"))
        copy2(
            os.path.join(fname.replace("_in", "_gt")),
            os.path.join(args.output, "gt", str(i) + "_gt.png"),
        )
