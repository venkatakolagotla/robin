...
async def save_img(link: str, path: str):
    ...
    cv2.imwrite(
        path,
        cv2.imdecode(
            np.asarray(bytearray(urlopen(link).read()), dtype="uint8"), cv2.IMREAD_COLOR
        ),
    )
...
def parse_args():
    ...
    parser.add_argument(
        "-e", "--end", type=int, default=0, help=r"ending number (default: %(default)s)"
    )
    parser.add_argument(
        "-o",
        "--output",
