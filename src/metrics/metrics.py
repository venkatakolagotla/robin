...
bad_os_str = (
    r"""This script is platform-dependent. It can be run only on Microsoft Windows."""
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--weights",
        type=str,
        default=os.path.join("weights", "weights.exe"),
    ...
    parser.add_argument(
        ...
        "--metrics",
        type=str,
        default=os.path.join("metrics", "metrics.exe"),
...
def main():
    ...
    fnames_out = list(
        glob.iglob(os.path.join(args.input, "**", "*_out.*"), recursive=True)
    )
    with open(os.path.join(args.output, "total_res.txt"), "w") as res:
        total_res = reduce(
            lambda a, b: a + b,
            Pool(args.processes).map(
                partial(
                    meter, weights=args.weights, metrics=args.metrics, data=args.data
                ),
                fnames_out,
            ),
        )
